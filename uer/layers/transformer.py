...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_attn = MultiHeadedAttention(
            args.hidden_size, args.heads_num, attention_head_size, args.dropout, has_bias=has_bias, with_scale = with_scale,weight_squeeze=self.weight_squeeze
        )
        ...
        if args.feed_forward == "gated":
            ...
            self.feed_forward = PositionwiseFeedForward(
                args.hidden_size, args.feedforward_size, args.hidden_act, has_bias,weight_squeeze=self.weight_squeeze
            )
        ...
        if args.layernorm == "t5":
            ...
            self.layer_norm_1 = LayerNorm(args)
            self.layer_norm_2 = LayerNorm(args)

    def forward(self, hidden, mask,layer_num,mask_dict,current_step, position_bias = None, has_residual_attention=False, prev_attn=None):
        """
        Args:
        ...
        if self.layernorm_positioning == "post":
            ...
            inter = self.layer_norm_1(inter + hidden,"module.encoder.transformer.{}.self_attn.final_linear_right_compactor.weight".format(layer_num),mask_dict,current_step)
            output = self.dropout_2(self.feed_forward(inter))
            output = self.layer_norm_2(output + inter,"module.encoder.transformer.{}.feed_forward.linear_2_right_compactor.weight".format(layer_num),mask_dict,current_step)
        else:
