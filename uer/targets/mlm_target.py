...
class MlmTarget(nn.Module):
    ...
    def __init__(self, args, vocab_size):
        ...
        if self.factorized_embedding_parameterization:
            ...
            if args.weight_squeeze:
                self.mlm_linear_1_left_compactor = nn.Linear(args.hidden_size, args.hidden_size,bias=False)
                self.mlm_linear_1_right_compactor = nn.Linear(args.hidden_size, args.hidden_size,bias=False)
                self.mlm_linear_2_left_compactor = nn.Linear(args.hidden_size, args.hidden_size, bias=False)
    ...
    def mlm(self, memory_bank, tgt_mlm,mask_dict,current_step):
        if self.weight_squeeze:
            memory_bank = self.mlm_linear_1_left_compactor(memory_bank)
            output_mlm = self.act(self.mlm_linear_1_right_compactor(self.mlm_linear_1(memory_bank)))
        else:
            output_mlm = self.act(self.mlm_linear_1(memory_bank))
        output_mlm = self.layer_norm(output_mlm,"module.target.mlm_linear_1_right_compactor.weight",mask_dict,current_step)
        if self.factorized_embedding_parameterization:
        ...
        if self.weight_squeeze:
            output_mlm = self.mlm_linear_2(self.mlm_linear_2_left_compactor(output_mlm))
        else:
    ...
    def forward(self, memory_bank, tgt, seg,mask_dict,current_step,soft_tgt):
        """
        Args:
        ...
        loss, correct, denominator = self.mlm(memory_bank, tgt,mask_dict,current_step)
